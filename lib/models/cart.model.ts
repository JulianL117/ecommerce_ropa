...
export class CartModel {
  ...
  static addItem(
    ...
    const detalleExistente = detallesCarrito.find(
      (d) => d.id_carrito === carrito.id_carrito && d.id_producto === id_producto && d.tallaSeleccionada === tallaSeleccionada
    );
   ...
   */
  static updateQuantityByDetalle(
    id_detalle: string,
    cantidad: number
  ): { success: boolean; carrito?: CarritoCompleto; message: string } {
    const detalle = detallesCarrito.find(
      (d) => d.id_detalle === id_detalle
    );
    ...
    if (cantidad <= 0) {
      return this.removeItemByDetalle(id_detalle);
    }

    // Validar stock
    const inventario = inventarios.find((i) => i.id_producto === detalle.id_producto);
    if (!inventario || inventario.stock < cantidad) {
      return { success: false, message: 'Stock insuficiente' };
    }
    ...
    // Obtener carrito para retornar
    const carrito = detallesCarrito.find((d) => d.id_detalle === id_detalle)?.id_carrito;
    const usuario = carrito ? detallesCarrito.find(d => d.id_carrito === carrito) : null;
    const id_usuario = usuario ? detallesCarrito.find(d => d.id_carrito === carrito)?.id_carrito : '';
   ...
   */
  static removeItemByDetalle(id_detalle: string): { success: boolean; message: string } {
    const index = detallesCarrito.findIndex(
      (d) => d.id_detalle === id_detalle
    );
