...
let cachedTallaColumn: boolean | null = null;

async function supportsTallaColumn(): Promise<boolean> {
  if (cachedTallaColumn !== null) {
    return cachedTallaColumn;
  }
  ...
  try {
    ...
    cachedTallaColumn = Number(rows[0]?.count || 0) > 0;
  } catch (error) {
    console.error('[CartModel] No se pudo verificar columna tallaSeleccionada:', error);
    cachedTallaColumn = false;
  }
...
export class CartModelMySQL {
  ...
  static async getCartCompleto(id_usuario: string): Promise<CarritoCompleto> {
    ...
    const items: DetalleCarrito[] = rows.map((row) => ({
      ...
      tallaSeleccionada: row.tallaSeleccionada,
  ...
  static async addItem(
    ...
    const hasTallaColumn = await supportsTallaColumn();
    const tallaParam = tallaSeleccionada === undefined ? null : tallaSeleccionada;

    // Buscar si ya existe el producto con la misma talla en el carrito
    const detalleExistente = await query<DetalleCarritoRow[]>(
      hasTallaColumn
        ? 'SELECT * FROM detalle_carrito WHERE id_carrito = ? AND id_producto = ? AND (tallaSeleccionada = ? OR (tallaSeleccionada IS NULL AND ? IS NULL))'
        : 'SELECT * FROM detalle_carrito WHERE id_carrito = ? AND id_producto = ?',
      hasTallaColumn
        ? [carrito.id_carrito, id_producto, tallaParam, tallaParam]
    ...
    if (detalleExistente.length > 0) {
      ...
      if (hasTallaColumn) {
        await query<ResultSetHeader>(
          'INSERT INTO detalle_carrito (id_detalle, cantidad, id_carrito, id_producto, tallaSeleccionada) VALUES (?, ?, ?, ?, ?)',
          [id_detalle, cantidad, carrito.id_carrito, id_producto, tallaSeleccionada === undefined ? null : tallaSeleccionada]
        );
      } else {
        await query<ResultSetHeader>(
  ...
  /**
   * Actualizar cantidad por id_detalle
   */
  static async updateQuantityByDetalle(
    id_usuario: string,
    id_detalle: string,
    cantidad: number
  ): Promise<{ success: boolean; carrito?: CarritoCompleto; message: string }> {
    if (cantidad <= 0) {
      return this.removeItemByDetalle(id_usuario, id_detalle);
    }

    // Validar stock primero
    const detalle = await query<DetalleCarritoRow[]>(
      'SELECT id_producto FROM detalle_carrito WHERE id_detalle = ?',
      [id_detalle]
    );
  ...
  /**
   * Eliminar item por id_detalle
   */
  static async removeItemByDetalle(
    id_usuario: string,
    id_detalle: string
  ): Promise<{ success: boolean; carrito?: CarritoCompleto; message: string }> {
    const result = await query<ResultSetHeader>(
