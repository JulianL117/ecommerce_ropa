...
let cachedTallaPedidoColumn: boolean | null = null;

async function supportsTallaPedidoColumn(): Promise<boolean> {
  if (cachedTallaPedidoColumn !== null) {
    return cachedTallaPedidoColumn;
  }
    ...
    const rows = await query<RowDataPacket[]>(
      `SELECT COUNT(*) as count
       ...
       WHERE table_schema = DATABASE()
         ...
         AND column_name = 'tallaSeleccionada'`
    );

    cachedTallaPedidoColumn = Number(rows[0]?.count || 0) > 0;
  } catch (error) {
    console.error('[OrderModel] No se pudo verificar columna tallaSeleccionada en detalle_pedido:', error);
    cachedTallaPedidoColumn = false;
  }
...
export class OrderModelMySQL {
  ...
  static async createFromCart(
    ...
    try {
      const hasTallaColumn = await supportsTallaPedidoColumn();

      // Iniciar transacción
      const queries: { sql: string; params: unknown[] }[] = [];

        // Crear pedido
        const pedidoColumns = ['id_pedido', 'id_usuario'];
        const pedidoParams: unknown[] = [id_pedido, id_usuario];

        if (datosPago.direccion_envio) {
          pedidoColumns.push(
            'direccion_calle',
            'direccion_ciudad',
          ...
          pedidoParams.push(
            datosPago.direccion_envio.calle || null,
            datosPago.direccion_envio.ciudad || null,
            datosPago.direccion_envio.departamento || null,
            datosPago.direccion_envio.codigo_postal || null,
            datosPago.direccion_envio.telefono || null,
            datosPago.direccion_envio.referencias || null
          );
        }

        queries.push({
          sql: `INSERT INTO pedido (${pedidoColumns.join(', ')}) VALUES (${pedidoColumns
            .map(() => '?')
            .join(', ')})`,
          params: pedidoParams,
        });
        ...
        for (const item of carrito.items) {
          ...
          if (hasTallaColumn) {
            queries.push({
              sql: `INSERT INTO detalle_pedido (id_detalle, cantidad, precio, id_pedido, id_producto, tallaSeleccionada) VALUES (?, ?, ?, ?, ?, ?)`,
              params: [
                id_detalle,
                item.cantidad,
            ...
            queries.push({
              sql: `INSERT INTO detalle_pedido (id_detalle, cantidad, precio, id_pedido, id_producto) VALUES (?, ?, ?, ?, ?)`,
              params: [
                id_detalle,
                item.cantidad,
                item.producto?.precio || 0,
  ...
  static async findById(id: string): Promise<Pedido | null> {
    ...
    return {
      ...
      direccion_envio: pedido.direccion_calle
        ? {
            calle: pedido.direccion_calle,
            ciudad: pedido.direccion_ciudad || "",
            departamento: pedido.direccion_departamento || "",
            codigo_postal: pedido.direccion_codigo_postal || "",
            telefono: pedido.direccion_telefono || "",
            referencias: pedido.direccion_referencias || undefined,
          }
        : undefined,
