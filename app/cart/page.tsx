...
export default function CartPage() {
  ...
  const handleUpdateQuantity = async (id_detalle: string, newQuantity: number) => {
    setUpdatingItems((prev) => new Set(prev).add(id_detalle));
    await updateQuantity(id_detalle, newQuantity);
    setUpdatingItems((prev) => {
      const newSet = new Set(prev);
      newSet.delete(id_detalle);
      return newSet;
    });
  };

  const handleRemove = async (id_detalle: string) => {
    setUpdatingItems((prev) => new Set(prev).add(id_detalle));
    await removeFromCart(id_detalle);
    setUpdatingItems((prev) => {
      const newSet = new Set(prev);
      newSet.delete(id_detalle);
      return newSet;
              ...
              <div className="flex gap-4 p-4">
                ...
                <div className="flex flex-col items-end justify-between">
                  <Button
                    ...
                    disabled={updatingItems.has(item.id_detalle)}
                    className="text-muted-foreground hover:text-destructive"
                  >
                    {updatingItems.has(item.id_detalle) ? (
                  ...
                  <div className="flex items-center border rounded-md">
                    <Button
                      ...
                      className="h-8 w-8"
                      onClick={() =>
                        handleUpdateQuantity(item.id_detalle, item.cantidad - 1)
                      }
                      disabled={
                        item.cantidad <= 1 ||
                        updatingItems.has(item.id_detalle)
            ...
            <CardContent className="space-y-4">
              ...
              {carrito.total < 100000 && (
                <p className="text-xs text-muted-foreground">
                  Agrega {formatPrice(100000 - carrito.total)} más para envío
                  gratis
                </p>
              )}
            </CardContent>
