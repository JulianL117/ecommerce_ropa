...
export async function POST(request: NextRequest) {
  try {
    ...
    const resultado = await CategoryModelMySQL.create(nombre, imagen);
...
export async function PUT(request: NextRequest) {
  try {
    const body = await request.json();
    const { id_categoria, nombre, imagen } = body;
    ...
    const resultado = await CategoryModelMySQL.update(id_categoria, nombre, imagen);
